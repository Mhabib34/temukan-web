...
import { Home, FileText, Plus, User, LogIn, Map } from "lucide-react";
...
export function BottomNav() {
  ...
  if (!isHydrated) {
    return (
      <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
        ...
        <div className="relative flex items-end justify-around px-2 pb-safe pt-1 h-14" />
      </nav>
    );
  }
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
      ...
      <div className="absolute inset-0 bottom-0 top-2 bg-white/80 backdrop-blur-md border-t border-stone-200" />
            ...
            return (
              ...
              >
                <button
                  ...
                  className="-translate-y-4 flex focus:outline-none h-14 w-14 items-center justify-center rounded-full bg-orange-500 text-white shadow-lg shadow-orange-200 transition-transform duration-150 active:scale-95 hover:bg-orange-600"
                >
                  {tab.icon}
                </button>
              </div>
          ...
          const content = (
            ...
            >
              {tab.label === "Notifikasi" && unreadCount > 0 && (
                <span className="absolute -top-0.5 right-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-bold leading-none text-white">
